...
var _slideShow = __webpack_require__(329);

__webpack_require__(331);

var firstSliderElement = document.querySelector('.myslideshow');

var mySlideShow2 = new _slideShow.SlideShowClass(firstSliderElement, 2000);
...
exports.SlideShowClass = undefined;
...
var SlideShowClass = function () {
    function SlideShowClass(sliderRootElement, intervalTime) {
        _classCallCheck(this, SlideShowClass);
    ...
    _createClass(SlideShowClass, [{
        ...
        value: function init() {
            ...
            this.renderPager();
            this.renderControls();
            this.showSlide(); //to synchronize
            this.startInterval(); // function which starts interval
            this.sliderRootElement.addEventListener('mouseenter', function () {
                _this.stopInterval();
            });
            this.sliderRootElement.addEventListener('mouseleave', function () {
                _this.startInterval();
            });
            this.addTouchEvents();
        ...
        value: function addTouchEvents() {
            ...
            this.sliderRootElement.addEventListener('touchstart', function (e) {
                e.preventDefault();
                _this2.stopInterval();
                _this2.touchCoordinates.start = e.touches[0].clientX;
            });
            this.sliderRootElement.addEventListener('touchmove', function (e) {
                var dif = _this2.touchCoordinates.start - e.touches[0].clientX;
                var difAtPercent = dif * (100 / _this2.rootElementWidth);

                console.log(_this2.rootElementWidth, dif, difAtPercent);
                _this2.moveSlide(difAtPercent);
            });
            this.sliderRootElement.addEventListener('touchend', function (e) {
                var dif = _this2.touchCoordinates.start - e.changedTouches[0].clientX;
                if (dif > 0) {
                    _this2.nextSlide();
                } else {
                    _this2.prevSlide();
        ...
        key: 'moveSlide',
        value: function moveSlide(translateAmount) {
        ...
        value: function showSlide() {
            this.slidesList.style.transform = 'translate(-' + this.activeSlideIndex * 100 + '%)'; //moves slider forward
        }
    }, {
        key: 'toggleOnPagerItem',
        value: function toggleOnPagerItem(currentActiveSlideIndex) {
            this.pagerItems.forEach(function (item) {
                return item.classList.remove('slideshow__pager-item_active');
            });
        ...
        value: function renderPager() {
            ...
            var _loop = function _loop(i) {
                // пробегаем по DOM page - children
                var pagerItem = document.createElement('li');

                pagerItem.classList.add('slideshow__pager-item'); //creating n-oe количество pager-item depends on amount of sliders

                if (i === _this3.activeSlideIndex) {
                    // base on 'trafic lights work' we add active class
                    pagerItem.classList.add('slideshow__pager-item_active');
                }

                pagerItem.addEventListener('click', function () {
                    // on each li hanging click to move slider
                    _this3.activeSlideIndex = i;
                    _this3.toggleOnPagerItem(i);
                    _this3.showSlide();
                });

                _this3.pagerItems.push(pagerItem);
                ul.appendChild(pagerItem); // inserting li in ul
            };
            ...
            this.sliderRootElement.appendChild(ul); // inserting ul child to rootElement
        }
    }, {
        key: 'renderControls',
        ...
        key: 'renderSlide',
        value: function renderSlide(slideContent) {
        ...
        value: function wrap() {
            ...
            this.slides = Array.from(this.sliderRootElement.children).map(function (slideContent) {
                var slide = _this5.renderSlide(slideContent);
                _this5.slidesList.appendChild(slide);
            });

            viewPort.append(this.slidesList);

            this.sliderRootElement.innerHTML = ''; // обнуляем content
            this.sliderRootElement.appendChild(viewPort); // changing content to viewPort
    ...
    return SlideShowClass;
}();

exports.SlideShowClass = SlideShowClass; // SlideShow
